...
class BaseCameraSubscriber(Node):

    def __init__(self, arg1='base_camera_subscriber'):
        super().__init__(arg1)
        self.subscription = self.create_subscription(
            sensor_msgs.msg.Image,
            '/base_rgb_frame', #'joint_states',
...
def main(args=None):
    ...
    base_camera_subscriber = BaseCameraSubscriber()

    rclpy.spin(base_camera_subscriber)

    # Destroy the node explicitly
    # (optional - otherwise it will be done automatically
    # when the garbage collector destroys the node object)
    base_camera_subscriber.destroy_node()
    rclpy.shutdown()
